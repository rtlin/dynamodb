...
export class NotesComponent {
    constructor(notesApiService, notesDataService) {
    ...
    ngOnInit() {
        ...
        this.alert = {};
        this.refreshNotes();
        this.AddNoteSubscription = this.notesDataService.addNote$.subscribe((note) => {
            this.addNote(note);
        });
    }
    refreshNotes() {
        this.isLoading = true;
        this.userNotes = [];
        this.notesApiService.getNotes().subscribe(res => {
            let data = res;
    ...
    onScrollDown() {
        ...
        this.NotesDataSubscription = this.notesApiService.getNotes(this.startKey).subscribe(res => {
            let data = res;
            if (_.has(res, 'LastEvaluatedKey')) {
                this.startKey = res.LastEvaluatedKey.timestamp;
            }
